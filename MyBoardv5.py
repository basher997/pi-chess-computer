...
def SaveGame(result):                
    ...
    if BoardSlot != -1:
        ...
        if white == True:
            fn= player.rstrip() +' v StockFish13 Pi Level '+str(skill)+' '
        else:
            fn='StockFish13 Pi Level '+str(skill)+' v '+ player.rstrip() +' '
...
def LoadSettings():
    ...
    if path.exists(os.path.join(sys.path[0],'Board.cfg')):
        with open(os.path.join(sys.path[0],'Board.cfg'),'r') as f:  
...
def SaveSettings():
    ...
    if os.path.exists(sys.path[0]+'/Board.cfg'):
        os.remove (sys.path[0]+'/Board.cfg')
        print('Delete Old Board.cfg')
    
    with open(os.path.join(sys.path[0],'Board.cfg'),'x') as f: 
        f.write(str(skill)+'\n')
        f.write(str(movet)+'\n')
        f.write(str(white)+'\n')
        f.write(player+'\n')
