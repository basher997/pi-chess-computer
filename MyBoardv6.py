...
# This program uses the MUX TCA9548A with I2C address x70.
...
#
#
# Copyright 2021 Alan Bishop and Max Drobes
#
...
#start chessboard - checks for valid moves etc
ALchess = ChessBoard()
...
timemoves = ['1sec  ','3sec  ','6sec  ','10sec ','15sec ','20sec ','25sec ','30sec ',' 1min ',' 2min ',' 3min ',' 5min ']
#set move time 0=1sec  1=3sec etc
movet = 1
#set white = true black = false
white = True
#set skill level
skill = 1
#set sleep time pause after pushing button
slptime = 0.4
#set player name
player = 'Player'
#set save slot
...
# MCPs on channels 3, 4, 5, 6, 7 
    ...
    SW.chn(i2c_channel) # switch mux channel
...
def ReadButtons():
    ...
    # no button pressed = 255
    # green button pressed = 254
    # chrome button pressed  = 253
    # red button pressed = 251
    # black button pressed = 247
    
    SW.chn(7) # switch mux channel
...
def ReadBoard():
  # read the 8 registers
    ...
    for k in range(0,4):  
        ...
        for l in range(2):  # for each MCP register A and B
            ...
            if a != mbrd[x]: # there has been a change
                ...
                zp = 88 # set to return a piece has been put down
                
                if a > mbrd[x]:
                    zp = 99   #A piece has been picked up
    ...
    #get the state of the reedswitches store in boardReed()
    for k in range(0,8):
        bp=1
        a=mbrd[k]
        for l in range(0,8):
...
def printb():  # used to debug show state of board and reeds
    print ("  +-----------------+")
...
def getbrdmove():
    ...
    while moveone[2] != 99 and butp != 247: #if no piece picked up and quit button NOT been pressed
        butp = CheckButtons() #check and deal with buttons
        moveone = ReadBoard() #check for board change
    
    if butp == 247: #black button pressed QUIT
        mymove = 'quit'
    ...
    if mymove != 'quit':
        ...
        if sqone == 1:
            ...
            capturetype = 1
            print('capturetype 1 remove opponents piece - pick up your piece - put down your piece')
    ...
    ##second piece move~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        if mymove != 'quit':
            ...
            if invalidmove != True:
                ...
                while movetwo[2] < 88: # wait for piece to be put down
                    butp = CheckButtons()
                    movetwo = ReadBoard()
            ...
            #ledmx(moveone[0],moveone[1],0)
                        print('Piece replace Move Two Set Invalid Move')
                ...
                #ledmx(movetwo[0],movetwo[1],1)
                        print('capturetype 2 pick up your piece - remove opponents piece - put down your piece')
...
def CheckButtons():
    ...
    if gbutp == 254: #green button verify
        ...
        PlaySound('Your Move',False)
        
    if gbutp == 253: #chrome hint
        time.sleep(slptime)
        if lhint != '':
            Gamemessage(2,'I Suggest '+lhint)
...
def PlayerMove():
    ...
    if bmessage[0] == 'm':
        if ALchess.addTextMove(brdmove) == False and ALchess.getReason() != 5:
            ...
            PlaySound(bmessage[1]+' '+bmessage[2]+instr+bmessage[3]+' '+bmessage[4],False)
...
def SaveGame(result):                
    ...
    fdis = ''
    BoardSlot = GetBoardSlot()
    if BoardSlot != -1:
        slot = BoardSlot
    
        GetPlayerName()
    
        fullpath = os.path.join(sys.path[0],'saveslots/'+str(slot))
        try:
            os.mkdir(fullpath)
        except OSError:
        ...
        fdis = fdis + 'L'+str(skill)
        
        fdis = fdis + ' '
        
        fdis = fdis + td.strftime("%d")
        
        fdis = fdis + '-'
        
        fdis = fdis + td.strftime("%B")[0:3]
        ...
        if result == 1:
            ...
            fdis = fdis + 'D'
        #end of prepare description
            
        # write game description to file
        if path.exists(fullpath+str(slot)+'.dis'):
            os.remove (fullpath+str(slot)+'.dis')
        
        f = open(fullpath+str(slot)+'.dis', "x")
        f.write(fdis)
        f.close()
        # game description saved
        # write pgn to file 
        if path.exists(fullpath+str(slot)+'.pgn'):
            os.remove (fullpath+str(slot)+'.pgn')
         
        
        
        f = open(fullpath+str(slot)+'.pgn', "x")
        f.write('[Event "N/A"]\n')
        f.write('[Site "Oxted, England GBR"]\n')
        f.write('[Date "'+ td.strftime("%Y.%m.%d") + '"]\n')
        f.write('[Round "N/A"]\n')
        if white == True:
            f.write('[White "'+player.rstrip()+'"]\n')
            f.write('[Black "StockFish13 Pi"]\n')   
            if result == 6:
                f.write('[Result "0-1"]\n')
        else:
            f.write('[White "StockFish13 Pi"]\n')
            f.write('[Black '+ player.rstrip() +']\n')
            if result == 6:
                f.write('[Result "1-0"]\n')
        ...
        #pgn file saved
        # write fen file
        if path.exists(fullpath+str(slot)+'.fen'):
            os.remove (fullpath+str(slot)+'.fen')
        
        f = open(fullpath+str(slot)+'.fen', "x")
        
        f.write(ALchess.getFEN())
        f.close()
        #fen file saved
        
        if path.exists(fullpath+str(slot)+'.fmo'):
            os.remove (fullpath+str(slot)+'.fmo')
        
        f = open(fullpath+str(slot)+'.fmo', "x")
        
        f.write(fmove)
        f.close()
        
        SaveSettings(fullpath)    
...
def GetBoardSlot():
    global slot
    slottemp = slot
    LCDmenu(1,'Choose Slot','Hort','Vert','Select','Exit')
    
    LedSlot(slottemp,1)
    butp = 255
    slotchoose = False
    
    if path.exists(os.path.join(sys.path[0],'saveslots/'+str(slottemp)+'/'+str(slottemp)+'.pgn')):
        matrix.blink_rate = 0
    else:
        matrix.blink_rate = 2
    LedSlot(slottemp,1)
    ...
    while slotchoose == False:
    
        butp = ReadButtons()
       
        
        if butp == 254: #green button move to right
            print('Add One To Slot')
            print('slottemp '+str(slottemp))
            
            print ('slottemp % 8 '+str(slottemp % 8))
            LedSlot(slottemp,0)
            ...
            if slottemp % 8 == 7:
                ...
                slottemp = slottemp + 1
            print(os.path.join(sys.path[0],'saveslots/'+str(slot)+'/'+str(slot)+'.pgn'))
            ...
            if path.exists(os.path.join(sys.path[0],'saveslots/'+str(slottemp)+'/'+str(slottemp)+'.pgn')):
                ...
                matrix.blink_rate = 2
            LedSlot(slottemp,1)
            print('Add One To Slot '+str(slottemp))
            time.sleep(slptime) 
        
        if butp == 253: #chrome button go up row
            LedSlot(slottemp,0)
            if slottemp + 8 > 63:
                slottemp = slottemp -56
            else:
                slottemp = slottemp + 8
            ...
            LedSlot(slottemp,1)
            print('Go Up Row '+ str(slottemp))
            time.sleep(slptime)
        
        if butp == 251: #Red button select this slot
            print('Select Slot '+ str(slottemp))
            slotchoose = True      
            LedSlot(slottemp,0)
        
        if butp == 247: #Black button EXIT
            print ('EXIT')
            slotchoose = True
            LedSlot(slottemp,0)
            slottemp =-1
...
def LoadSettings(fpath):
    global skill
    global movet
    global white
    global player
    
    if path.exists(fpath + 'Board.cfg'):
        with open(fpath + 'Board.cfg','r') as f:  
            
            skill = int(f.readline())
            print('skill = '+str(skill))
            
            movet = int(f.readline())
            print('movetime = '+ str(movet))
...
def SaveSettings(fpath):
    global skill
    global movet
    global white
    global player
    
    if os.path.exists(fpath +'/Board.cfg'):
        os.remove (fpath +'/Board.cfg')
        print('Delete Old Board.cfg')
    
    with open(fpath + '/Board.cfg','x') as f: 
        f.write(str(skill)+'\n')
        f.write(str(movet)+'\n')
        f.write(str(white)+'\n')
        f.write(player+'\n')
...
#main menu
    #print(GetBoardSlot())
    LoadSettings(os.path.join(sys.path[0]))
    LCDmenu(1,'Choose Option','Play','Setup','Load','Shutdown')
    gpress =ReadButtons()
    while gpress == 255:
        gpress = ReadButtons()
    ...
    #load save setup position menu   
        if white==True:
            ...
            cside = 'Black'
        LCDclear()
        LCDmenu(1,'Game Menu','Start',cside,'Lvl '+str(skill),timemoves[movet])
    ...
    #Gamemenu
        SaveSettings(os.path.join(sys.path[0]))
    ...
    #ALchess.setFEN('B1k4N/3b4/2p3P1/1p5Q/p7/P1P5/1P5P/R6K w - - 1 31')
        ...
        while result == 0:
            ...
            if result != 0:
                ...
                LCD(3,11,'Save Game')
                butp = 255
                while butp == 255:# 255 = no buttons pressed
                    butp = ReadButtons()
                
                    if butp == 247:#black button pushed save game
                        SaveGame(result)
                        LCD(2,5,'Game Saved')
                        time.sleep(slptime*3)
                        LCDclear()
                        LCD(2,5,LcdText)
                        ...
                        time.sleep(slptime*4)
                       
                        butp = 255
                    elif butp != 254:
                        butp = 255
            
    if gpress == 253: #setup board position
         print('setup')

    if gpress == 251: #load game
    
        print('Load Game')
        
    if gpress == 247:#black button
        print('Shutdown')